...
public class AutoCompletionHandler{
	...
	public void activate(){
		...
		try{
			...
			rs = db.executeQuery("SELECT * FROM PICKWAYPOINT WHERE pickjob = 1 AND section="+section);
		
		
			while(rs.next()){
				Waypoint w = new Waypoint(rs.getFloat(3), rs.getFloat(4), rs.getInt(2), rs.getString(5), rs.getInt(6), rs.getInt(7), rs.getInt(8));
				path.add(w);
			}
	...
	public void update(){
		...
		if(action == GETONPJ){
			...
			if(!player.inVehicle()){
				...
				//player.getVehicleBeingUsed().rotateUpTo(new Vector3f());
			}
			...
			if ((Math.abs(myX - x) < .1f) && (Math.abs(myZ - z) < .1f)) {
				...
				walking = false;
				if ((myX < x) && (Math.abs(myX - x) > .07f)) {
					player.getVehicleBeingUsed().setLocalTranslation(myX + .070f, .1f, myZ);
				...
				if ((myZ < z) && Math.abs(myZ - z) > .07f) {
					player.getVehicleBeingUsed().setLocalTranslation(myX, .1f, myZ + .070f);
				} else if ((myZ > z) && Math.abs(myZ - z) > .07f) {
					player.getVehicleBeingUsed().setLocalTranslation(myX, .1f, myZ - .070f);
				}
			}
