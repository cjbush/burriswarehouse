//
// $Id: BRootNode.java,v 1.2 2007/04/27 19:46:29 vivaldi Exp $
//
...
public abstract class BRootNode extends Geometry {
    ...
    public ArrayList<BWindow> getAllWindows() {
        return _windows;
    }
    ...
    /**
     * Configures the number of seconds that the mouse must rest over a component to trigger a tooltip. If the value is
     * set to zero, tips will appear immediately.
     */
    ...
    /**
     * Sets the preferred width of tooltip windows. The default is to prefer a width slightly less wide that the entire
     * window.
     */
    ...
    /**
     * This is called by a window or a scroll pane when it has become invalid. The root node should schedule a
     * revalidation of this component on the next tick or the next time an event is processed.
     */
    public abstract void rootInvalidated(BComponent root);

    /**
     * Configures a component to receive all events that are not sent to some other component. When an event is not
     * consumed during normal processing, it is sent to the default event targets, most recently registered to least
     * recently registered.
     */
    ...
    /**
     * A large component that changes its tooltip while it is the hover component in the normal course of events can
     * call this method to force an update to the tooltip window.
     */
    ...
    public void updateGeometricState(float time,
        ...
        if (tiptext == null) {
            throw new RuntimeException("tiptext was never initialized");
        }
    ...
    /**
     * Dispatches an event to the specified target (which may be null). If the target is null, or did not consume the
    ...
    /**
     * Called by a window when its position changes. This triggers a recomputation of the hover component as the window
     * may have moved out from under or under the mouse.
     */
